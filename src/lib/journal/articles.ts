import { Article } from "./types";
import { BATCH_1 } from "./batches/batch-1";
import { BATCH_2 } from "./batches/batch-2";
import { BATCH_3 } from "./batches/batch-3";
import { BATCH_4 } from "./batches/batch-4";

const ORIGINAL_ARTICLES: Article[] = [
  {
    slug: "what-is-a-biblical-money-type",
    title: "What Is a Biblical Money Type? A Framework for Faithful Stewardship",
    excerpt:
      "Most Christians have never been given language for how they relate to money. The Biblical Money Type framework changes that — by naming four distinct patterns of stewardship found in Scripture.",
    date: "May 15, 2026",
    category: "Framework",
    readTime: "6 min read",
    content: `
      <p>Most Christians know what the Bible says about money. Tithe. Avoid debt. Give generously. Be content. But knowing the commands is not the same as knowing <em>yourself</em> — and without self-knowledge, the commands become either oppressive or performative.</p>

      <p>The Biblical Money Type framework exists to close that gap. It is not a personality test. It is not a spiritual gift assessment. It is a stewardship lens — a way of seeing how God has wired you to relate to resources, risk, rest, and responsibility.</p>

      <h3>Four Types, One Calling</h3>

      <p>Scripture presents at least four distinct financial archetypes: <strong>Solomon the Visionary</strong>, who multiplied wealth through wisdom and long-horizon thinking. <strong>Joseph the Guardian</strong>, who stored against famine with discipline and foresight. <strong>The Macedonians the Givers</strong>, who gave out of extreme poverty with overflowing joy. <strong>Nehemiah the Builder</strong>, who finished what others abandoned through systems and perseverance.</p>

      <p>Each was faithful. None was the same. And that is the point.</p>

      <p>The Visionary sees opportunity where others see risk. The Guardian sees danger where others see abundance. The Giver sees need before it is spoken. The Builder sees systems where others see chaos. Your type is not your destiny — it is your starting point. It names what is already true so you can steward it more faithfully.</p>

      <h3>Why This Matters</h3>

      <p>When you do not know your type, you default to whatever culture tells you. For some, that means chasing wealth without wisdom. For others, it means hoarding out of fear. For still others, it means giving past your means or building systems no one asked for.</p>

      <p>Knowing your type gives you three things: <strong>clarity</strong> about why you make the money decisions you make. <strong>language</strong> for conversations with your spouse, advisor, or pastor. And <strong>a path</strong> — specific, actionable, rooted in Scripture — toward the faithful steward God has called you to be.</p>

      <h3>How to Discover Yours</h3>

      <p>The <a href="/quiz">Talanthos Biblical Money Type assessment</a> takes three to four minutes. It asks fifteen questions about your instincts, your regrets, your emotions, and your hopes. At the end, you receive a personalized profile: your primary type, your four-dimensional score, and a thirty-day action plan designed specifically for how God wired you.</p>

      <p>It is free. It is grounded in Scripture. And it might be the most clarifying four minutes you spend this year.</p>
    `,
  },
  {
    slug: "the-visionary-solomons-legacy",
    title: "The Visionary: How Solomon's Legacy Applies to Modern Finance",
    excerpt:
      "Solomon had wisdom and wealth — and lost both when vision outran counsel. What can the modern Visionary learn from his rise, his reign, and his fall?",
    date: "May 12, 2026",
    category: "Type Profile",
    readTime: "7 min read",
    content: `
      <p>Solomon asked God for wisdom, and God gave him wealth, honor, and influence beyond measure. He built the temple, expanded trade routes, and authored three thousand proverbs. He was the original Visionary — a man who saw what could be and made it real.</p>

      <p>But Solomon also had seven hundred wives and three hundred concubines. He built high places for foreign gods. And by the end of his life, the wisdom that had built his wealth was no longer governing it. Vision outran counsel. Multiplication became excess. And the kingdom fractured.</p>

      <h3>The Modern Visionary</h3>

      <p>If you are a Visionary, you recognize Solomon's pattern in your own life. You see opportunities before others do. You think in decades, not quarters. You are drawn to ventures, investments, and initiatives that multiply what has been entrusted to you. And you are often right.</p>

      <p>But your greatest strength is also your greatest risk. When vision moves faster than wisdom, you outrun the people God has placed around you. You defer rest because the next thing is more exciting. You confuse God's blessing with your own brilliance. And slowly, imperceptibly, the vision that was meant to serve becomes the ambition that consumes.</p>

      <h3>Three Guardrails for the Visionary</h3>

      <p><strong>First, establish a wisdom council.</strong> Solomon wrote Proverbs with input from others. He did not make decisions alone. Find two or three people who can slow you down without shutting you down. Bring them your biggest opportunity before you bring it to the market.</p>

      <p><strong>Second, schedule rest as seriously as you schedule meetings.</strong> The Visionary's temptation is to defer rest until the vision is realized. But the vision is never fully realized. Rest is not a reward for finishing. It is a rhythm for continuing.</p>

      <p><strong>Third, distinguish multiplication from accumulation.</strong> Solomon multiplied temples, wives, and wealth. But he did not multiply faithfulness. Ask yourself: is this next venture about stewarding what God has given, or about proving what I can build?</p>

      <h3>Discover Your Type</h3>

      <p>Not sure if you are a Visionary? <a href="/quiz">Take the Talanthos assessment</a> and find out. In under four minutes, you will know your primary type, your four-dimensional score, and the specific blind spots that are most likely to derail your financial faithfulness.</p>
    `,
  },
  {
    slug: "the-guardian-when-saving-becomes-hoarding",
    title: "The Guardian: When Saving Becomes Hoarding — and How to Open the Storehouses",
    excerpt:
      "Joseph stored grain for seven years. Then he opened the storehouses. The Guardian's greatest risk is not scarcity. It is the paralysis of never opening the door.",
    date: "May 10, 2026",
    category: "Type Profile",
    readTime: "6 min read",
    content: `
      <p>Joseph interpreted Pharaoh's dream, foresaw seven years of plenty followed by seven years of famine, and built a storage system that saved a nation. He was the original Guardian — a man who saw what others missed and prepared accordingly.</p>

      <p>But here is what we often forget: Joseph stored for seven years, and then he <em>opened the storehouses</em>. The famine was the very reason he gathered. The storage had a purpose beyond storage. And that is where many modern Guardians get stuck.</p>

      <h3>The Guardian's Gift</h3>

      <p>If you are a Guardian, you have a rare and needed gift. You see risk before it arrives. You build margin when others spend it. You protect dependents, team members, and communities with a discipline that looks like foresight. In volatile seasons, you are the calm voice. In abundant seasons, you are the wise restraint.</p>

      <p>Your love language is runway. You sleep better with six months of expenses in the bank. You have modeled the worst-case scenario so many times that it no longer frightens you. And people trust you with their resources because you treat them as trusts, not as toys.</p>

      <h3>The Guardian's Trap</h3>

      <p>But stewardship can quietly harden into hoarding. Fear of loss can eclipse the calling to give. And the same instinct that protected you in the lean years can paralyze you in the plenty.</p>

      <p>The question for the Guardian is not "Do I have enough?" It is "Am I deploying what I have stored for the purpose God intended?" Joseph did not gather grain to admire it. He gathered it to feed people. And at some point, the storehouse door had to open.</p>

      <h3>Three Practices for the Guardian</h3>

      <p><strong>First, name your storehouses.</strong> What are you storing — money, time, energy, relationships — and for whom? Write it down. If the answer is "just in case," dig deeper. In case of what? And for whom?</p>

      <p><strong>Second, set deployment dates.</strong> Joseph opened the storehouses when the famine began. He did not wait until it was over. Set calendar dates for when you will release what you have stored: a gift, an investment, a conversation, a risk.</p>

      <p><strong>Third, find a Visionary or Giver as a counterweight.</strong> Guardians need people around them who will push them to open the door. Your caution is a gift. But left unchecked, it becomes a cage.</p>

      <p><a href="/quiz">Take the Talanthos assessment</a> to discover if you are a Guardian — and to receive a personalized thirty-day plan for moving from storage to sending.</p>
    `,
  },
  {
    slug: "the-giver-generosity-without-burnout",
    title: "The Giver: Generosity Without Burnout — A Biblical Balance",
    excerpt:
      "The Macedonians gave out of extreme poverty. But they also gave out of overflow. What does sustainable generosity look like for the modern Giver?",
    date: "May 8, 2026",
    category: "Type Profile",
    readTime: "6 min read",
    content: `
      <p>Paul writes about the Macedonian churches with awe. In the midst of a very severe trial, their overflowing joy and their extreme poverty welled up in rich generosity. They gave not because they had abundance, but because they had grace. They were the original Givers.</p>

      <p>But Paul also writes something else, just a few verses later. He commends the Corinthians for their willingness, but then he adds: "I am not commanding you, but I want to test the sincerity of your love by comparing it with the earnestness of others." And later: "Each of you should give what you have decided in your heart to give, not reluctantly or under compulsion, for God loves a cheerful giver."</p>

      <p>Even in Scripture, generosity needs boundaries.</p>

      <h3>The Giver's Gift</h3>

      <p>If you are a Giver, you know this instinct in your bones. You give early. You give first. You give past comfort. You read need in a room before it is spoken. And you find a peculiar joy in sending resources out that others would hold onto.</p>

      <p>Your giving is not transactional. It is identity-level. You do not give to get a tax break or a name on a plaque. You give because money feels most alive when it is moving toward someone else's need. You have, perhaps without knowing it, discovered one of the deepest truths in Scripture: it is more blessed to give than to receive.</p>

      <h3>The Giver's Deception</h3>

      <p>But the Giver's greatest deception is believing that generosity excuses structure. That grace replaces budgeting. That saying yes to every need is somehow more spiritual than saying yes to the right needs.</p>

      <p>The Macedonians gave out of poverty — but they gave <em>as they were able</em>. Paul did not praise them for bankrupting themselves. He praised them for giving out of their means, not beyond them. And that is the distinction that saves the Giver from burnout.</p>

      <h3>The Floor and Ceiling Framework</h3>

      <p><strong>Floor:</strong> The minimum provision for those entrusted to you. Housing, food, health, transportation. Below this line, giving is not generosity. It is self-harm.</p>

      <p><strong>Ceiling:</strong> The maximum you give without consulting your household, your budget, or your advisor. Above this line, giving is not obedience. It is evasion — a way to manage guilt, avoid hard decisions, or feel spiritual without doing the work of discernment.</p>

      <p><strong>The space between:</strong> This is where generosity lives. In between floor and ceiling, you give freely, joyfully, and sustainably.</p>

      <p><a href="/quiz">Take the Talanthos assessment</a> to discover your giving patterns, your blind spots, and a personalized strategy for sustainable generosity.</p>
    `,
  },
  {
    slug: "the-builder-systems-that-outlast-you",
    title: "The Builder: Building Systems That Outlast You",
    excerpt:
      "Nehemiah built with a trowel in one hand and a sword in the other. But he also stopped to read the law aloud. What can the modern Builder learn about systems, rest, and human connection?",
    date: "May 5, 2026",
    category: "Type Profile",
    readTime: "7 min read",
    content: `
      <p>Nehemiah heard that the walls of Jerusalem were broken down and its gates burned with fire. He wept. He prayed. And then he built. In fifty-two days, he restored what had been in ruins for over a century. He was the original Builder — a man who translated conviction into systems, plans, and finished work.</p>

      <p>But Nehemiah did not build alone. He organized families by district. He assigned sections of the wall to specific groups. He armed the builders while they worked. And when the wall was finished, he gathered the people and read the law aloud — because he knew that walls without worship become prisons.</p>

      <h3>The Modern Builder</h3>

      <p>If you are a Builder, you finish what you start. You translate vision into spreadsheets, budgets, and timelines. You are patient, methodical, and resilient against opposition. While others chase the new, you restore the broken. While others abandon projects at 80%, you cross the finish line.</p>

      <p>Your money obeys the blueprint, not the mood. You have an investment policy, a budget, a giving schedule, and a quarterly review. You do not need motivation. You need a plan. And once you have one, you execute with a consistency that compounds over decades.</p>

      <h3>The Builder's Wall</h3>

      <p>But systems can crowd out spontaneity. Plans can become identity. And the walls you build to protect what matters can become barriers that keep people out.</p>

      <p>The Builder's blind spot is loving the blueprint more than the people in the house. You may finish the budget, the estate plan, the investment strategy — and wonder why no one is left inside to celebrate. Your systems are meant to serve people. When people start serving the systems, something has inverted.</p>

      <h3>Three Practices for the Builder</h3>

      <p><strong>First, schedule the pause before the next phase.</strong> Nehemiah stopped to read the law. He did not rush from wall-building to city-governance without a moment of recalibration. Build a "pause ritual" into every project: a meal, a prayer, a day of rest before the next blueprint.</p>

      <p><strong>Second, invite people into the system, not just under it.</strong> Your budget is not just for you. It is for your family, your church, your community. Share it. Explain it. Let others see the blueprint so they can help you build.</p>

      <p><strong>Third, celebrate completion.</strong> Builders are quick to start the next phase and slow to mark the finish. But celebration is not indulgence. It is gratitude. It is witness. It is the declaration that God, not you, built the wall.</p>

      <p><a href="/quiz">Take the Talanthos assessment</a> to discover if you are a Builder — and to receive a personalized plan for building systems that include rest, celebration, and human connection.</p>
    `,
  },
  {
    slug: "debt-and-the-gospel",
    title: "Debt and the Gospel: A Biblical Framework for Financial Freedom",
    excerpt:
      "The Bible does not forbid debt. But it warns about it constantly. What does a gospel-centered approach to debt actually look like in practice?",
    date: "May 1, 2026",
    category: "Biblical Finance",
    readTime: "8 min read",
    content: `
      <p>The Bible mentions debt over one hundred times. Almost every mention is a warning. The borrower is slave to the lender. The wicked borrow and do not repay. Do not wear yourself out to get rich. And yet, the Bible never explicitly forbids debt. It warns, it limits, it disciplines — but it does not ban.</p>

      <p>This tension confuses many Christians. Is debt a sin? Is a mortgage okay? What about student loans? Business leverage? The answer depends on your type, your season, and your stewardship posture. But there are principles that apply to every believer.</p>

      <h3>Debt as Bondage</h3>

      <p>Proverbs 22:7 is the clearest verse on debt: "The rich rule over the poor, and the borrower is slave to the lender." Debt is not just a financial arrangement. It is a power relationship. When you borrow, you give someone else a claim on your future income, your future decisions, and your future peace.</p>

      <p>This does not mean all debt is sin. But it means all debt is serious. The question is not "Is this debt allowed?" The question is "Does this debt serve my calling, or does it enslave my future?"</p>

      <h3>Debt as Tool</h3>

      <p>Scripture also presents debt as a tool in specific contexts. In the Old Testament, lending was a form of generosity — but with strict limits. No interest to fellow Israelites. No perpetual debt. Jubilee every fifty years, when all debts were forgiven and all land returned.</p>

      <p>The principle is this: debt should be temporary, purposeful, and bounded. A mortgage to house your family is different from credit card debt for a vacation. A business loan to create jobs is different from a car loan for status. The same dollar figure can be wise or foolish depending on the purpose.</p>

      <h3>A Gospel-Centered Approach</h3>

      <p>The gospel does not just give us rules about debt. It gives us identity. You are not defined by your net worth, your credit score, or your debt-to-income ratio. You are defined by Christ's finished work. And from that identity, you make financial decisions with freedom rather than fear.</p>

      <p>Here is a practical framework:</p>

      <p><strong>1. Name every debt.</strong> Write them all down: balance, interest rate, minimum payment, and payoff date. Denial is the enemy. Clarity is the beginning of freedom.</p>

      <p><strong>2. Stop adding.</strong> Do not take on new debt until the existing debt has a plan. This is not legalism. It is discipline.</p>

      <p><strong>3. Choose a method.</strong> Avalanche (highest interest first) saves money. Snowball (smallest balance first) wins momentum. Choose one and commit.</p>

      <p><strong>4. Build margin.</strong> Even while paying off debt, keep a small emergency fund. A flat tire should not become a credit card charge.</p>

      <p><strong>5. Celebrate milestones.</strong> Every debt paid off is a small exodus. Mark it. Thank God. Then keep going.</p>

      <p><a href="/quiz">Take the Talanthos assessment</a> to discover how your Biblical Money Type shapes your relationship with debt — and to receive a personalized debt strategy for your specific wiring.</p>
    `,
  },
  {
    slug: "why-every-christian-should-know-their-money-type",
    title: "Why Every Christian Should Know Their Money Type",
    excerpt:
      "Self-knowledge is not vanity. It is stewardship. And when it comes to money, most believers have never been given a mirror.",
    date: "April 28, 2026",
    category: "Stewardship",
    readTime: "5 min read",
    content: `
      <p>Jesus tells a parable about three servants who receive talents from their master. One receives five, one receives two, one receives one. The first two invest and multiply. The third buries his in the ground. When the master returns, he praises the first two and condemns the third.</p>

      <p>But here is what we often miss: the master does not praise the five-talent servant for receiving five. He praises him for being faithful with five. The two-talent servant receives the same praise. The issue was never the amount. It was the faithfulness.</p>

      <p>And yet, most Christians spend their entire financial lives comparing their five talents to someone else's two. Or worse, they spend their lives trying to become a two-talent servant when God made them a five. They adopt budgets that do not fit their instincts. They follow investment advice that contradicts their wiring. They feel guilty for giving too little or too much, saving too little or too much, risking too little or too much — because they have never been given a mirror.</p>

      <h3>The Mirror of Self-Knowledge</h3>

      <p>Knowing your Biblical Money Type is not about labeling yourself. It is about seeing yourself. It is the difference between walking through a dark room and turning on a light. The furniture has not moved. But now you can navigate it.</p>

      <p>When you know your type, you understand why you make the money decisions you make. You stop feeling guilty for instincts that are actually gifts. You name blind spots that have been sabotaging you for years. And you gain a shared language for conversations with your spouse, your financial advisor, your pastor, and your children.</p>

      <h3>Four Reasons to Take the Assessment</h3>

      <p><strong>1. Clarity.</strong> Most financial advice is generic. "Save 15%. Give 10%. Avoid debt." But what if you are a Giver who needs to learn to save? What if you are a Guardian who needs to learn to deploy? Generic advice applied to the wrong type creates guilt, not growth.</p>

      <p><strong>2. Communication.</strong> Money is the leading cause of marital conflict. Not because couples disagree about amounts, but because they disagree about meaning. The Visionary sees a investment. The Guardian sees a risk. The Giver sees a need. The Builder sees a system. Knowing your types gives you a shared vocabulary.</p>

      <p><strong>3. Accountability.</strong> Blind spots are patterns so familiar that you no longer notice them. The Visionary who outruns counsel. The Guardian who never opens the storehouse. The Giver who bleeds past the budget. The Builder who finishes alone. A type-aware accountability partner can name what you cannot see.</p>

      <p><strong>4. Growth.</strong> The goal is not to become another type. It is to become more faithfully who God already made you to be. The assessment gives you a thirty-day action plan — specific, daily micro-steps calibrated to your wiring.</p>

      <p><a href="/quiz">Take the Talanthos Biblical Money Type assessment</a> today. It is free. It takes under four minutes. And it might be the most clarifying conversation you have ever had about money.</p>
    `,
  },
  {
    slug: "30-day-stewardship-challenge",
    title: "The 30-Day Stewardship Challenge: One Small Step Per Day",
    excerpt:
      "Transformation does not happen in a single decision. It happens in thirty small ones. Here is a month-long challenge designed to reshape your relationship with money — one day at a time.",
    date: "April 25, 2026",
    category: "Practical",
    readTime: "5 min read",
    content: `
      <p>Most Christians want to be better stewards. Few know where to start. The problem is not desire. It is direction. We read books, listen to podcasts, and make resolutions — but without a daily rhythm, change does not stick.</p>

      <p>The 30-Day Stewardship Challenge is designed to solve that. One action per day. None takes more than ten minutes. The goal is not to transform your finances in a month. It is to transform your posture — from reactive to intentional, from anxious to faithful.</p>

      <h3>Week 1: Seeing</h3>

      <p><strong>Day 1:</strong> Write down your current net worth. Assets minus liabilities. No judgment. Just a number.</p>

      <p><strong>Day 2:</strong> Read Psalm 24:1. "The earth is the Lord's, and everything in it." List three things you are stewarding that are not money.</p>

      <p><strong>Day 3:</strong> Review your last thirty days of spending. Highlight every transaction that aligned with your values. Notice the patterns.</p>

      <p><strong>Day 4:</strong> Write a one-sentence mission statement for your money this year. What is it for?</p>

      <p><strong>Day 5:</strong> Identify one subscription or recurring expense you have not reviewed in six months. Cancel or keep with intention.</p>

      <p><strong>Day 6:</strong> Calculate your giving percentage last year. No shame. Just a number.</p>

      <p><strong>Day 7:</strong> Rest. No financial task today. Just gratitude.</p>

      <h3>Week 2: Naming</h3>

      <p><strong>Day 8:</strong> Write down your biggest financial regret. Then write one thing you learned from it.</p>

      <p><strong>Day 9:</strong> <a href="/quiz">Take the Talanthos Biblical Money Type assessment</a>. Discover your primary type, your scores, and your blind spots.</p>

      <p><strong>Day 10:</strong> Share your type with one person: spouse, friend, or advisor. Ask what blind spot they see most clearly.</p>

      <p><strong>Day 11:</strong> Read one chapter of Proverbs. Underline every verse about money, work, or wisdom.</p>

      <p><strong>Day 12:</strong> Write down three things you are grateful for that money cannot buy.</p>

      <p><strong>Day 13:</strong> Identify one financial fear. Pray specifically about it.</p>

      <p><strong>Day 14:</strong> Rest. No financial task today. Just presence.</p>

      <h3>Week 3: Moving</h3>

      <p><strong>Day 15:</strong> Set one automatic transfer: to savings, to giving, or to debt repayment. Let the system outrun your mood.</p>

      <p><strong>Day 16:</strong> Give anonymously today. No receipt, no tax benefit, no story.</p>

      <p><strong>Day 17:</strong> Read your bank statement from last month. Circle every impulse purchase. Note the emotion that preceded it.</p>

      <p><strong>Day 18:</strong> Schedule a quarterly financial review with yourself. Put it on the calendar.</p>

      <p><strong>Day 19:</strong> Write a letter to your future self about money. Seal it. Open in one year.</p>

      <p><strong>Day 20:</strong> Identify one person who needs what you have — time, skill, money, or connection. Make the transfer.</p>

      <p><strong>Day 21:</strong> Rest. No financial task today. Just worship.</p>

      <h3>Week 4: Sending</h3>

      <p><strong>Day 22:</strong> Review your investment allocation. Does it match your time horizon and values?</p>

      <p><strong>Day 23:</strong> Calculate your runway: how many months could you survive with no income?</p>

      <p><strong>Day 24:</strong> Eliminate one financial complexity. One account, one subscription, one commitment.</p>

      <p><strong>Day 25:</strong> Review your will or estate plan. If you do not have one, schedule the first step.</p>

      <p><strong>Day 26:</strong> Give to a cause you have never supported before. Stretch your circle.</p>

      <p><strong>Day 27:</strong> Write down three ways God has provided for you in the past. Read them when fear rises.</p>

      <p><strong>Day 28:</strong> Re-take the <a href="/quiz">Talanthos assessment</a>. See if anything has shifted.</p>

      <p><strong>Day 29:</strong> Share what you learned with someone else. Stewardship multiplies when it is passed on.</p>

      <p><strong>Day 30:</strong> Rest. No financial task today. Just the knowledge that God owns it all — and He is faithful.</p>
    `,
  },
];

export const ARTICLES: Article[] = [
  ...ORIGINAL_ARTICLES,
  ...BATCH_1,
  ...BATCH_2,
  ...BATCH_3,
  ...BATCH_4,
];

export function getArticleBySlug(slug: string): Article | undefined {
  return ARTICLES.find((a) => a.slug === slug);
}

export function getAllArticles(): Article[] {
  return ARTICLES;
}
