...
export async function POST(req: NextRequest) {
  const limit = rateLimit(req, { max: 10, windowMs: 60_000, keyPrefix: "landing-view" });
  if (!limit.success) {
  ...
  try {
    ...
    const { error } = await supabase.from("landing_views").insert({
      ip_address: ip,
    ...
    if (error) {
      console.error("[LANDING VIEW ERROR]", error);
